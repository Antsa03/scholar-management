...
export default function RootLayout({
  ...
  const isEtudiantLoggedIn = user?.role === "Etudiant";
      ...
      >
        <Provider>
          {isLoginPathname ? (
            ...
            <div
              className={`flex flex-row w-full relative  custom-mobile:-bg--bg-primary-color dark:-bg--bg-dark-secondary-color  ${
                isEtudiantLoggedIn ? "justify-center min-h-screen" : ""
              }`}
            >
              <Sidebar isSidebarOpen={isSidebarOpen} />
              <main
                className={`flex flex-col  gap-2  shadow-gray-600/50 z-0  transition-all ${
                  isEtudiantLoggedIn ? "w-full 2xl:w-3/4" : "w-full flex-grow"
                }`}
              >
                <button
                ...
                <div
                  className={`hidden  lg:flex flex-row items-center justify-between gap-4 py-2  pl-12 pr-12 2xl:pr-8 h-[80px] -bg--bg-primary-color  border-gray-300 ${
                    isEtudiantLoggedIn ? "shadow-md" : "border-b-[1px]"
                  }`}
                >
                  <TopbarContainer
                    darkMode={darkMode}
