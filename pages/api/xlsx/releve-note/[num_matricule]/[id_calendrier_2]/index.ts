    ...
    if (
      ...
      allNotes.unite_enseignements.map((ue: UE_note) => {
        ...
        ue.matieres.map((matiere: Matiere_note) => {
          validation_1 = "";
          coeff_somme += parseFloat(matiere.coeff);
          moy_ue +=
            parseFloat(matiere.coeff) * parseFloat(matiere.note_matiere);
          if (parseFloat(matiere.note_matiere) < 5) validation_1 = "Non validé";
        });
        moy_ue = moy_ue / coeff_somme;
        moy_ue_tab.push(moy_ue.toFixed(2));
        validation =
      ...
      const excel = await generateExcel(data, "releve-note-excel");
